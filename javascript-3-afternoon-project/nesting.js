...
//Code Here
//this code works in Repl
function recordCleaner(obj){
  let rows = obj.accidents.filter(record => {
    if(record.atFaultForAccident === true){
...
//Code Here
//This works fine in Repl.it and the console.
function looper(arr){
  let rows = arr.length;
  for(let i=0; i<rows; i++){
    let items = arr[i].length;
